...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (data.ok) {
      setMessage(
        "<p>Cool, got it!</p><p>Will get back to you as soon as I can 💪🏼</p>"
      );
    } else {
      setMessage(
        "<p>Whoa, that didn't work! Sorry about that. Please try again soon.</p>"
      );
    }
  };
  ...
  const showMessage = () =>
    message && (
      <div className="my-5 text-muted font-italic">{renderHTML(message)}</div>
    );
